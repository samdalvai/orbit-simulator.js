...
import { BodyRenderStyle } from '../src/view/BodyRenderStyle';

declare const process: {
    on(event: 'exit', listener: () => void): void;
};

const bodyRenderStyles = new Map<number, BodyRenderStyle>();
const engine = new Engine(bodyRenderStyles);
...
for (let i = 0; i < numBodies; i++) {
    ...
    const z = randomNumber(1e-8, 1e8);
    const radius = randomNumber(10_000, 100_000);
    const mass = randomNumber(1e16, 1e24);
    addNewBody(x, y, z, radius, mass, BodyType.PLANET, new Vec3(), NO_PARENT);
}

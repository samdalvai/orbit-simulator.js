...
const ASTEROID_BELT_OBJECTS = 3_500;
const KUIPER_BELT_OBJECTS = 5_000;
...
export function createSolarSystem(engine: Engine): SolarSystem {
    ...
    const addBody = (body: Body, renderStyle: BodyRenderStyle) => {
        const packedId = engine.addBody(body);
        ...
        bodies.push(body);
        renderStyles.set(packedId, renderStyle);
    };

    const sun = createBody(SUN, BodyType.STAR);
    addBody(sun, createRenderStyle(SUN, BodyType.STAR));

    for (const planetSpec of PLANETS) {
        const planet = createBody(planetSpec, BodyType.PLANET, sun);
        addBody(planet, createRenderStyle(planetSpec, BodyType.PLANET));

        for (const moonSpec of planetSpec.moons ?? []) {
            const moon = createBody(moonSpec, BodyType.MOON, planet);
            addBody(moon, createRenderStyle(moonSpec, BodyType.MOON));
        }
    }

    for (const beltSpec of BELTS) {
        for (const asteroid of createBelt(sun, beltSpec)) {
            addBody(asteroid.body, asteroid.renderStyle);
        }
    }
...
function createBelt(sun: Body, spec: BeltSpec): CreatedBody[] {
